...
class AcousticViewer {
    ...
    setupEventListeners() {
        ...
        if (generateBtn) {
            generateBtn.onclick = () => this.generateDoppler();
        }

        // Doppler Estimation
        const estimateBtn = document.getElementById('estimateVelocity');
        if (estimateBtn) {
            estimateBtn.onclick = () => this.estimateVelocity();
        }

        // Drone Detection
        const detectBtn = document.getElementById('detectDrone');
        if (detectBtn) {
            detectBtn.onclick = () => this.detectDrone();
        }

        // File Uploads
        const dopplerFile = document.getElementById('dopplerFile');
        if (dopplerFile) {
            dopplerFile.onchange = (e) => this.handleDopplerFile(e.target.files[0]);
        }

        const droneFile = document.getElementById('droneFile');
        if (droneFile) {
            droneFile.onchange = (e) => this.handleDroneFile(e.target.files[0]);
        }
        ...
        resultsDiv.innerHTML = `
            <div style="background: #0f1422; padding: 20px; border-radius: 8px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div>
                        <small style="color: #8a9ab0;">Frequency Range</small><br>
                        <strong>${data.frequency_range.min.toFixed(1)} - ${data.frequency_range.max.toFixed(1)} Hz</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Doppler Shift</small><br>
                        <strong>${data.doppler_shift.toFixed(1)} Hz</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Duration</small><br>
                        <strong>${data.duration} seconds</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Sample Rate</small><br>
                        <strong>${data.sample_rate} Hz</strong>
                    </div>
                </div>
    ...
    async estimateVelocity() {
        ...
        try {
            ...
            if (result.status === 'success') {
                this.displayVelocityEstimation(result.estimation);
            }
    ...
    displayVelocityEstimation(estimation) {
        ...
        resultsDiv.innerHTML = `
            <div style="background: #0f1422; padding: 20px; border-radius: 8px; margin-top: 20px;">
                ...
                <div style="font-size: 2rem; text-align: center; margin-bottom: 20px; color: #4a9eff;">
                    ${estimation.estimated_velocity.toFixed(1)} <span style="font-size: 1rem; color: #8a9ab0;">m/s</span>
                </div>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div>
                        <small style="color: #8a9ab0;">Approach Velocity</small><br>
                        <strong>${estimation.velocity_approach.toFixed(1)} m/s</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Recede Velocity</small><br>
                        <strong>${estimation.velocity_recede.toFixed(1)} m/s</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Max Freq Ratio</small><br>
                        <strong>${estimation.max_frequency_ratio.toFixed(3)}</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Min Freq Ratio</small><br>
                        <strong>${estimation.min_frequency_ratio.toFixed(3)}</strong>
                    </div>
                </div>
                ...
                <div style="margin-top: 15px; padding: 10px; background: rgba(74,158,255,0.1); border-radius: 5px;">
                    ...
                    <strong>${estimation.method}</strong>
    ...
    async detectDrone() {
        ...
        try {
            ...
            const response = await fetch(`${this.API_URL}/acoustic/drone/detect`, {
                method: 'POST',
                body: formData
            });
            ...
            if (result.status === 'success') {
                this.displayDroneDetection(result.detection);
            }
    ...
    displayDroneDetection(detection) {
        ...
        const isDrone = detection.detected;
        ...
        resultsDiv.innerHTML = `
            <div style="background: #0f1422; padding: 20px; border-radius: 8px; margin-top: 20px;">
                ...
                <div style="text-align: center; margin-bottom: 20px;">
                    ...
                    <div style="font-size: 1.5rem; color: ${color};">${detection.drone_type}</div>
                </div>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div>
                        <small style="color: #8a9ab0;">Confidence</small><br>
                        <strong>${detection.confidence.toFixed(1)}%</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Fundamental Freq</small><br>
                        <strong>${detection.fundamental_freq.toFixed(1)} Hz</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Spectral Centroid</small><br>
                        <strong>${detection.spectral_centroid.toFixed(1)} Hz</strong>
                    </div>
                    <div>
                        <small style="color: #8a9ab0;">Harmonic Ratio</small><br>
                        <strong>${(detection.harmonic_ratio * 100).toFixed(1)}%</strong>
                    </div>
                </div>
                
                <div style="margin-top: 15px; padding: 10px; background: rgba(${isDrone ? '81,207,102' : '255,107,107'}); border-radius: 5px;">
                    <strong style="color: ${color};">${detection.spectral_signature}</strong>
                </div>
            </div>
        `;
    }
    ...
    handleDopplerFile(file) {
        const label = document.querySelector('.file-label[for="dopplerFile"]');
        if (label) {
    ...
    handleDroneFile(file) {
        const label = document.querySelector('.file-label[for="droneFile"]');
        if (label) {
            label.innerHTML = `📁 Selected: ${file.name}`;
        }
    }
