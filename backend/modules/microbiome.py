    ...
    def __init__(self):
        ...
        def analyze_patient(self, bacterial_counts):
            ...
            try:
                ...
                if isinstance(bacterial_counts, dict):
                    ...
                    # Assume it's already a list/dict format
                    profile = bacterial_counts
                ...
                # Calculate Firmicutes/Bacteroidetes ratio (important biomarker)
                f_b_ratio = profile.get('Firmicutes', 0) / profile.get('Bacteroidetes', 1) if profile.get(
                    'Bacteroidetes', 0) > 0 else 0

                # Calculate diversity (Shannon index)
                diversity = self._calculate_diversity(profile)

                # Compare with disease profiles
                distances = {}
                for condition, ref_profile in self.disease_profiles.items():
                    # Calculate Jensen-Shannon distance (better for probability distributions)
                ...
                max_possible_distance = 1.0  # Maximum JS distance
        ...
        def _calculate_dysbiosis(self, profile):
            ...
            try:
                ...
                # - Low diversity
                # - High Proteobacteria
                # - Low Firmicutes/Bacteroidetes ratio (or extreme values)
                # - High Fusobacteria
                ...
                diversity_score = max(0, 1 - diversity / 3)  # Normal diversity ~2-3
                proteo_score = min(1, proteobacteria / 0.2)  # >20% is concerning
                fusobacter_score = min(1, fusobacteria / 0.1)  # >10% is concerning
                f_b_score = 0
                if f_b_ratio < 0.5 or f_b_ratio > 5:
                    f_b_score = min(1, abs(f_b_ratio - 2) / 10)  # Deviation from normal ~2
        ...
        def _identify_risk_factors(self, profile):
            ...
            # Check Proteobacteria (inflammation marker)
            ...
            # Check Firmicutes/Bacteroidetes ratio
        ...
        def get_sample_data(self, dataset='ihmp'):
            ...
            try:
                if dataset == 'ihmp':
                    ...
                    # Random sample
                    return {
                        'Firmicutes': random.uniform(0.3, 0.7),
                        'Bacteroidetes': random.uniform(0.1, 0.4),
                        'Proteobacteria': random.uniform(0.02, 0.2),
                        'Actinobacteria': random.uniform(0.01, 0.1),
                        'Fusobacteria': random.uniform(0, 0.08),
                        'Other': random.uniform(0, 0.05)
                    }
            except Exception as e:
